...
| H1c | 2026-09-13 | `62e73a3` (superseded `9defc5f`, which had the deprecated `huggingface-cli` bug) | `jobs/h1c_cifar_pets.yaml` (planted_cifar_pets ρ=0.99, 3 runs) | `E1-planted_cifar_pets-ef0cc3ab-s{0,1,2}` | prepared, estimate ~1.5-2h total (scaled from H1's measured per-image cost x ~4.2 for CIFAR's larger 10k-image train split -- not directly measured on cloud hardware) | not yet |
| H2 | | | | | | |
| H3 | | | | | | |
